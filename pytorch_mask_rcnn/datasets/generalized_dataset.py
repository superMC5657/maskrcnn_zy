...
class GeneralizedDataset:
    ...
    def _check(self, seq):
        ...
        for i in seq:
            img_id = self.ids[i]
            target = self.get_target(img_id) # get_target can't find declaration
            boxes = target["boxes"]
            labels = target["labels"]
            masks = target["masks"]

            try:
                assert len(boxes) > 0, "{}: len(boxs) = 0".format(i)
                assert len(boxes) == len(labels), "{}: len(boxs) != len(labels)".format(i)
                assert len(boxes) == len(masks), "{}: len(boxs) != len(masks)".format(i)
