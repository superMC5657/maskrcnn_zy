...
from pytorch_mask_rcnn.model.transform import Transform
from pytorch_mask_rcnn.model.utils import AnchorGenerator


class MaskRCNN(nn.Module, ABC):
    def __init__(self, backbone, num_classes,
        ...
        # ----transform----
        self.transform = Transform(min_size=800, max_size=1333, image_mean=[0.485, 0.456, 0.406],
    ...
    def forward(self, image, target=None):
        ...
        if self.training:
            ...
            result = self.transform.postprocess(result, image_shape, ori_image_shape)
            return result


class ResBackbone(nn.Module, ABC):
...
class FastRCNNPredictor(nn.Module, ABC):
...
class MaskRCNNPredictor(nn.Sequential, ABC):
    def __init__(self, in_channels, layers, dim_reduced, num_classes):

        order_dict = OrderedDict()
        next_feature = in_channels
        for layer_idx, layer_feature in enumerate(layers, 1):
            order_dict['mask_fcn{}'.format(layer_idx)] = nn.Conv2d(next_feature, layer_feature, 3, 1, 1)
            order_dict['relu{}'.format(layer_idx)] = nn.ReLU(inplace=True)
